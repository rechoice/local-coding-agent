...
schema_get_file_content = types.FunctionDeclaration(
    ...
    description="Reads and returns the contents of a file within a working directory, truncating the output if it exceeds a specified character limit",
    parameters=types.Schema(
        type=types.Type.OBJECT,
        properties={
            "file_path": types.Schema(
                type=types.Type.STRING,
                description="Path to the file to read, relative to the working directory (e.g., 'config.py', 'src/main.py')",
            ),
        },
        required=["file_path"],
    ),
